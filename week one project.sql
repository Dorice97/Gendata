...
CREATE TABLE jurisdiction_hierarchy (
    ...
    jurisdiction_name VARCHAR(50) NOT NULL,
...
SELECT
    d.jurisdiction_name AS District_Name,
    r.jurisdiction_name AS Region_Name,

#Calculate total individual beneficiaries
    SUM(
        CASE 
   ...
   #Calculate total individual beneficiaries / total population
    ROUND(
        CAST(SUM(
            CASE 
                WHEN b.beneficiary_type = 'Individuals' THEN b.beneficiaries
                ...
                ELSE 0
            END
        ) AS FLOAT) / SUM(v.total_population),
        4
    ) AS Beneficiaries_to_Population_Ratio

FROM jurisdiction_hierarchy AS vj
JOIN jurisdiction_hierarchy AS d 
    ON vj.jurisdiction_id = d.jurisdiction_id
JOIN jurisdiction_hierarchy AS r 
    ON d.jurisdiction_id = r.jurisdiction_id   
JOIN village_locations AS v 
    ON v.village = vj.jurisdiction_name 
JOIN beneficiary_partner_data AS b 
    ON b.village = v.village   

WHERE vj.jurisdiction_level = 'Village'
GROUP BY d.jurisdiction_name, r.jurisdiction_name;
select * 
...
SELECT
    ...
    COUNT(DISTINCT d.jurisdiction_name) AS Districts_Reached
FROM beneficiary_partner_data AS b
JOIN jurisdiction_hierarchy AS v 
    ON b.village = v.jurisdiction_name   
JOIN jurisdiction_hierarchy AS d 
    ON v.jurisdiction_id = d.jurisdiction_id     
WHERE v.jurisdiction_level = 'Village'
GROUP BY b.partner;
